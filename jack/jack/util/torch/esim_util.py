...
def get_mask(sequences_batch, sequences_lengths, device):
    """
    ...
    max_length = torch.max(sequences_lengths).item()
    mask = torch.ones(batch_size, max_length, dtype=torch.float)
    # mask[sequences_batch[:, :max_length] == 0] = 0.0
    seq_len_t = sequences_lengths.unsqueeze(1).expand(batch_size, max_length)
    max_len_t = torch.arange(max_length).unsqueeze(0).expand(batch_size, max_length)
    max_len_t = max_len_t.to(device)
    mask = seq_len_t > max_len_t
    return mask

...
class ESIM(nn.Module):
    ...
    def __init__(self,
                #  vocab_size,
                 embedding_dim=300, # for glove
                 hidden_size=300, # default for esim
                #  embeddings=None,
                 padding_idx=0,
                 dropout=0.5,
    ...
    def forward(self,
        ...
        premises_mask = get_mask(premises, premises_lengths, self.device).float()
        hypotheses_mask = get_mask(hypotheses, hypotheses_lengths, self.device).float()
        ...
        if self.self_attention:
            encoded_premises = self._premises_self_attention(encoded_premises, premises_mask)
            encoded_hypotheses = self._hypotheses_self_attention(encoded_hypotheses, hypotheses_mask)
        ...
        candidate_index = torch.argmax(logits, dim=1)

        # return logits, probabilities
        return logits, candidate_index
